  ...
  module ApplicationHelper
    ...
    def tabs(tabs = nil)
      ...
      if tabs
        ...
        tabs.each { |tab| tab[:active] = tab.key?(:url) && (@current_tab == tab[:text] || @current_tab == tab[:url][:controller]) }
      else
    ...
    def link_to_inline(id, url, options = {})
      ...
      text = (text) unless options[:plain]
      ...
      link_to(text,
              ...
              id: "#{id}_button",
              onclick: 
                "this.href = this.href.replace(/cancel=(true|false)/,'cancel='+ ($('##{id}').css('display') != 'none'));",
              class: options[:class])
    end
    ...
    def confirm_delete(model, params = {})
      model_name = model.class.to_s.gsub("FatFreeCrm::", "").downcase
      question = %(<span class="warn">#{t(:confirm_delete, model_name)}</span>)
    ...
    def link_to_add_fields(name, f, association)
      ...
      link_to(name, '#', class: "add_fields btn btn-primary mt-3", data: {id: id, fields: fields.gsub("\n", "")})
    end
  end
end
